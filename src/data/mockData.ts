import banner1 from '../assets/hero/banner-1.png';
import banner2 from '../assets/hero/banner-2.png';
import banner3 from '../assets/hero/banner-3.png';
import zolpidemImg from '../assets/medicine/Zolpidem.png';
import belbienImg from '../assets/medicine/belben 10mg.png';
import burfenImg from '../assets/medicine/burfen.png';
import tramadolRoyalImg from '../assets/medicine/Tramadol (Royal)-225mg.png';
import tramadolCitraImg from '../assets/medicine/thermadol(citra)-100mg.png';
import tramadolTrakemImg from '../assets/medicine/tramadol trakem 100mg.png';
import tapentadolImg from '../assets/medicine/Tapentadol (Aspadol-100mg).png';
import ritalinImg from '../assets/medicine/ritalin 30mg.png';
import firocitImg from '../assets/medicine/Firocit.png';
import gabapenteneImg from '../assets/medicine/Gabapentene.png';
import ativanImg from '../assets/medicine/Ativan (Lorezapam).png';
import somaImg from '../assets/medicine/soma 350mg.png';
import oxycodoneImg from '../assets/medicine/Oxycodone -30mg.png';
import valiumImg from '../assets/medicine/valium (diazepam) - 10mg.png';
import alprazolam2mgImg from '../assets/medicine/Alprazolam (xanax)- 2mg.png';
import alprazolam1mgImg from '../assets/medicine/Alprazolam (xanax)-1 mg.png';

// Hero Slides Data
export const heroSlides = [
    {
        id: 1,
        title: "Your Prescription for Affordable Health Solutions!",
        description: "Elevate your health journey with exclusive discounts and unparalleled convenience. Your path to well-being starts here, where every purchase is a prescription for savings.",
        image: banner1,
        isOffer: false,
        theme: 'dark'
    },
    {
        id: 2,
        preTitle: "Todays Hot Offer",
        title: "Unlock 50% Off on Essential Medicines!",
        description: "Embrace wellness without breaking the bank! Enjoy a generous 25% discount on a wide range of vital medicines at our online pharmacy. Your health matters, and so does your budget.",
        image: banner2,
        isOffer: true,
        theme: 'light'
    },
    {
        id: 3,
        title: "Your Prescription for Affordable Health Solutions!",
        description: "Elevate your health journey with exclusive discounts and unparalleled convenience. Your path to well-being starts here, where every purchase is a prescription for savings.",
        image: banner3,
        isOffer: false,
        theme: 'light'
    }
];

// Category Bar Data
export const categories = [
    "Pain Relief",
    "Cold and Flu",
    "Diabetes Care",
    "Digestive Health",
    "First Aid",
    "Skin Care",
    "Heart Health",
    "Respiratory Health"
];

// All Products Data
export const allProducts = [
    {
        "id": "belbien-10-mg",
        "name": "Belbien 10 mg",
        "metaTitle": "Buy Belbien 10 mg Online | Price $749 | Trusted Pharmacy",
        "metaDescription": "Buy Belbien 10 mg online at best price $749. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Belbien 10 mg"
        ],
        "image": belbienImg,
        "images": [
            belbienImg,
            belbienImg,
            belbienImg,
            belbienImg
        ],
        "boughtCount": "79 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "NOT RETURNABLE",
        "expiry": "2 Year",
        "composition": "Zolpidem Tartrate",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$749",
        "packaging": "180 Units",
        "unitPrice": "$4.16/unit",
        "usage": "Insomnia",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "NOT RETURNABLE",
            "Expires on or after": "March 2027",
            "Composition": "ZOLPIDEM TARTRATE",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Belbien 10 mg tablets Hemofarm are a medicine which is prescribed as a sleeping pill...",
        "uses": [
            {
                "title": "About Belbien 10 Tablet",
                "description": "Belbien 10 mg tablets Hemofarm are a medicine which is prescribed as a sleeping pill. It helps treat insomnia, i.e., when you have trouble getting and staying asleep. It helps you fall asleep more quickly and reduces your likelihood of waking up at night.\n\nIt comes in tablet form and can be obtained only on prescription.\n\nKey facts about Hemofarm Hemofarm Belbien Zolpidem 10 mg–\n\nHere are some key and important facts which you should know-\n\nThe drug takes around 1 hour to work.\nYou'll generally take it around a few weeks (up to 4 weeks).\nDo not drink alcohol or caffeine when you're taking Belbien 10 mg hemofarm tablet.\nIt is also called by the brand name Stilnoct."
            },
            {
                "title": "Who can and can't take zolpidem (belbien) 10 mg brand by hemofarm?",
                "description": "belbien zolpidem can be taken by most adults who are 18 and over and facing problems of insomnia.\n\nIt's not suitable for some people. To make sure zolpidem is safe for you, tell a doctor if you:\n\nMoreover, there are some limitations for taking the medicine. To know it is safe for you, consult your doctor if you\n\n• faced an allergic reaction to belbien zolpidem or any such drugs in the past.\n• have ongoing liver or kidney problems\n• have a medical condition that causes muscle weakness\n• have breathing problems\n• ever faced mental health problems\n• have ever had problems with alcohol or drugs\n• are breastfeeding, trying to get pregnant, or already pregnant"
            }
        ]
    },
    {
        "id": "zolpidem-10-mg",
        "name": "Zolpidem 10 mg",
        "metaTitle": "Buy Zolpidem 10 mg Online | Price $599 | Trusted Pharmacy",
        "metaDescription": "Buy Zolpidem 10 mg online at best price $599. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Zolpidem 10 mg"
        ],
        "image": zolpidemImg,
        "images": [
            zolpidemImg,
            zolpidemImg,
            zolpidemImg,
            zolpidemImg
        ],
        "boughtCount": "92 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "NOT RETURNABLE",
        "expiry": "2 Year",
        "composition": "Zolpidem Tartrate",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$599",
        "packaging": "180 Units",
        "unitPrice": "$4.16/unit",
        "usage": "Insomnia",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "ZOLPIDEM TARTRATE",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Zolpidem is used in the treatment of insomnia.",
        "uses": [
            {
                "title": "About Brufen 600 Tablet",
                "description": "Zolpidem is used in the treatment of insomnia."
            },
            {
                "title": "How Zolpidem works",
                "description": "Zolpidem belongs to a group of medicines called non-benzodiazepine hypnotics. It has hypnotic, sedative, and anticonvulsant (manages fits) properties. It works by binding of the GABAA receptor chloride channel macromolecular complex (a chemical messenger) that slows the activity of the nerve cells in the brain and helps fall asleep."
            },
            {
                "title": "Common side effects of Zolpidem",
                "description": "Most side effects do not require any medical attention and disappear as your body adjusts to the medicine. Consult your doctor if they persist or if you’re worried about them\n\nCommon\nDizziness, Sleepiness, Diarrhea, Drowsiness"
            }
        ]
    },
    {
        "id": "rivotril-2-mg",
        "name": "Rivotril 2 mg",
        "metaTitle": "Buy Rivotril 2 mg Online | Price $799 | Trusted Pharmacy",
        "metaDescription": "Buy Rivotril 2 mg online at best price $799. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Rivotril 2 mg"
        ],
        "image": burfenImg,
        "images": [
            burfenImg,
            burfenImg,
            burfenImg,
            burfenImg
        ],
        "boughtCount": "97 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "NOT RETURNABLE",
        "expiry": "2 Year",
        "composition": "Clonazepam 2 mg",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$799",
        "packaging": "180 Units",
        "unitPrice": "$4.16/unit",
        "usage": "Seizures, panic disorder",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "CLONAZEPAM (2MG)",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Rivotril 2mg Tablet is a medicine belonging to the benzodiazepine class. It is used to help control seizures and manage panic disorder. It calms overactive nerve signals in the brain and reduces abnormal electrical activity, leading to fewer seizures and less anxiety or panic.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Rivotril 2mg Tablet is a medicine belonging to the benzodiazepine class. It is used to help control seizures and manage panic disorder. It calms overactive nerve signals in the brain and reduces abnormal electrical activity, leading to fewer seizures and less anxiety or panic.\n\nRivotril 2mg Tablet should be taken exactly as your doctor prescribes. The dose may be adjusted slowly to find the amount that works best for you. Take it at the same time each day to maintain steady levels in the body. Do not stop taking the medicine suddenly. Stopping abruptly can lead to serious withdrawal symptoms, including seizures, mood changes, agitation, and unusual sensations. If treatment needs to be stopped, your doctor will guide you through a gradual dose reduction.\n\nThe medicine can cause dependence when used for long periods, and misuse or taking higher doses than prescribed can lead to serious harm. To get the most benefit, keep all follow-up appointments, inform your doctor about any other medicines you take, and immediately report worsening seizures, new mood changes, or unusual behavior.\n\nThe most common side effects include drowsiness, dizziness, fatigue, poor coordination or unsteady movements, problems with concentration or memory, and low mood. These effects usually appear when treatment is started, or doses are increased, and may lessen over time as the body adjusts. If any side effect becomes troublesome or severe, consult your doctor promptly.\n\nRivotril 2mg Tablet can impair thinking, judgment, and physical coordination. Activities like driving, operating machinery, or handling animals that require quick responses should be avoided until you know how the medicine affects you. Alcohol and other sedatives can dangerously increase drowsiness, breathing problems, or risk of overdose and should not be used with this medicine.\n\nPeople with liver disease, severe lung problems, or acute narrow-angle glaucoma should avoid using it. Individuals with depression or a history of mood disorders should be monitored closely because antiepileptic drugs, including Rivotril 2mg Tablet, may increase the risk of suicidal thoughts or behavior. If you are pregnant, planning to become pregnant, or breastfeeding, speak with your doctor, as the medicine can affect newborns and pass into breast milk."
            },
            {
                "title": "Uses of Rivotril Tablet",
                "description": "• Treatment of Seizures\n• Treatment of Panic disorder"
            },
            {
                "title": "Benefits of Rivotril Tablet",
                "description": "Rivotril 2mg Tablet helps reduce the frequency and severity of seizures. It works by enhancing the calming action in the brain, which helps stabilize abnormal electrical activity that triggers seizures. Better seizure control can lead to improved daily functioning, fewer disruptions to school or work, greater independence, and more confidence in routine activities."
            },
            {
                "title": "Side effects of Rivotril Tablet",
                "description": "Most side effects do not require any medical attention and disappear as your body adjusts to the medicine. Consult your doctor if they persist or if you’re worried about them\nCommon side effects of Rivotril\n• Drowsiness\n• Dizziness\n• Fatigue\n• Depression\n• Impaired coordination\n• Memory impairment"
            },
            {
                "title": "How to use Rivotril Tablet",
                "description": "Take this medicine in the dose and duration as advised by your doctor. Swallow it as a whole. Do not chew, crush or break it. Rivotril 2mg Tablet may be taken with or without food."
            }
        ]
    },
    {
        "id": "alprazolam-1-mg",
        "name": "Alprazolam (xanax)-1 mg",
        "metaTitle": "Buy Alprazolam (xanax)-1 mg Online | Price $699 | Trusted Pharmacy",
        "metaDescription": "Buy Alprazolam (xanax)-1 mg online at best price $699. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Alprazolam (xanax)-1 mg"
        ],
        "image": alprazolam1mgImg,
        "images": [
            alprazolam1mgImg,
            alprazolam1mgImg,
            alprazolam1mgImg,
            alprazolam1mgImg
        ],
        "boughtCount": "34 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "NOT RETURNABLE",
        "expiry": "2 Year",
        "composition": "Alprazolam",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$699",
        "packaging": "180 Units",
        "unitPrice": "$3.88/unit",
        "usage": "Anxiety, panic disorder",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "ALPRAZOLAM 1MG",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Alprazolam is used in the treatment of Anxiety and Panic disorder.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Alprazolam is used in the treatment of Anxiety and Panic disorder."
            },
            {
                "title": "How Alprazolam works",
                "description": "Alprazolam is a benzodiazepine. It works by increasing the action of a chemical messenger (GABA) which suppresses the abnormal and excessive activity of the nerve cells in the brain."
            },
            {
                "title": "Common side effects of Alprazolam",
                "description": "Most side effects do not require any medical attention and disappear as your body adjusts to the medicine. Consult your doctor if they persist or if you're worried about them\n\nCommon\nMemory Impairment, Sleepiness, Nausea, Constipation, Difficulty in speaking, Impaired coordination, Abnormality of voluntary movements, Decreased libido"
            }
        ]
    },
    {
        "id": "alprazolam-2-mg",
        "name": "Alprazolam (xanax)- 2mg",
        "metaTitle": "Buy Alprazolam (xanax)- 2mg Online | Price $799 | Trusted Pharmacy",
        "metaDescription": "Buy Alprazolam (xanax)- 2mg online at best price $799. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Alprazolam (xanax)- 2mg"
        ],
        "image": alprazolam2mgImg,
        "images": [
            alprazolam2mgImg,
            alprazolam2mgImg,
            alprazolam2mgImg,
            alprazolam2mgImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "NOT RETURNABLE",
        "expiry": "2 Year",
        "composition": "Alprazolam",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$799",
        "packaging": "180 Units",
        "unitPrice": "$4.16/unit",
        "usage": "Anxiety, panic disorder",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "ALPRAZOLAM-2MG",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Alprazolam is used in the treatment of Anxiety and Panic disorder.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Alprazolam is used in the treatment of Anxiety and Panic disorder."
            },
            {
                "title": "How Alprazolam works",
                "description": "Alprazolam is a benzodiazepine. It works by increasing the action of a chemical messenger (GABA) which suppresses the abnormal and excessive activity of the nerve cells in the brain."
            },
            {
                "title": "Common side effects of Alprazolam",
                "description": "Most side effects do not require any medical attention and disappear as your body adjusts to the medicine. Consult your doctor if they persist or if you're worried about them\n\nCommon\nMemory impairment, Sleepiness, Nausea, Constipation, Difficulty in speaking, Impaired coordination, Abnormality of voluntary movements, Decreased libido"
            }
        ]
    },
    {
        "id": "valium-10-mg",
        "name": "valium (diazepam) - 10mg",
        "metaTitle": "Buy valium (diazepam) - 10mg Online | Price $899 | Trusted Pharmacy",
        "metaDescription": "Buy valium (diazepam) - 10mg online at best price $899. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "valium (diazepam) - 10mg"
        ],
        "image": valiumImg,
        "images": [
            valiumImg,
            valiumImg,
            valiumImg,
            valiumImg
        ],
        "boughtCount": "76 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "NOT RETURNABLE",
        "expiry": "2 Year",
        "composition": "Diazepam",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$899",
        "packaging": "180 Units",
        "unitPrice": "$4.99/unit",
        "usage": "Anxiety, epilepsy, muscle spasm",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "DIAZEPAM-10MG",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Valium 10mg Tablet is a prescription medicine used in the treatment of short-term anxiety.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Valium 10mg Tablet is a prescription medicine used in the treatment of short-term anxiety. It can also be used to treat acute alcohol withdrawal, to relieve muscle spasm and as adjunctive therapy for seizures. This medicine relaxes the nerve cells and calms the brain.\n\nValium 10mg Tablet may be taken with or without food. However, take it at the same time each day as this helps to maintain a consistent level of medicine in the body. Take this medicine in the dose and duration as advised by your doctor as it has a high potential of habit-forming. If you have missed any doses, take it as soon as you remember it and finish the full course of treatment even if you feel better. Do not stop taking this medicine suddenly without talking to your doctor as it can cause withdrawal effects like depression and difficulty sleeping.\n\nThe most common side effects of this medicine include drowsiness, fatigue, muscle weakness, and impaired coordination. If any of these side effects do not subside on their own or get worse, let your doctor know. Also, do not drive or do anything that requires mental focus until you know how this medicine affects you.\n\nThis medicine may cause weight gain. But, a well balanced diet and regular exercise can help to reduce extra weight. If you have been taking this medicine for a long time then regular monitoring of blood and liver functions may be required."
            },
            {
                "title": "Uses of Valium Tablet",
                "description": "• Epilepsy\n• Short term anxiety\n• Alcohol withdrawal\n• Muscle spasm"
            },
            {
                "title": "Benefits of Valium Tablet",
                "description": "In Epilepsy\nValium 10mg Tablet slows down electrical signals in the brain which cause seizures (fits). It can also help reduce symptoms such as confusion, uncontrollable jerking movements, loss of awareness, and fear or anxiety. The medicine can allow you to do some activities that you would otherwise be forbidden or scared to do (such as swimming and driving). It can take a few weeks for this medicine to work (because the dose has to be increased slowly) and during this time you may still have seizures.\n\nIn Short term anxiety\nValium 10mg Tablet reduces the symptoms of excessive anxiety and worry that are triggered only at the time of stressful conditions that last for a shorter duration. These could be job interviews, exams, stage performances, etc. It can effectively manage feelings of restlessness, tiredness, difficulty concentrating and feeling irritable. Valium 10mg Tablet will therefore help you go about your daily activities more easily and be more productive. Keep taking this medicine even if you feel well. Stopping suddenly can cause serious problems.\n\nIn Alcohol withdrawal\nValium 10mg Tablet is a medicine used, along with social support and counseling, to help people recover from alcohol addiction and to avoid having the urge to drink alcohol again. Valium 10mg Tablet starts working from the very first dose and is a very safe and effective way of treating alcohol addiction.\n\nIn Muscle spasm\nValium 10mg Tablet effectively relieves muscle stiffness or spasm, thereby improving muscle movement. Take it as prescribed by the doctor to get the most benefit. Do not take more or for longer than needed as that can be dangerous and it also has habit-forming potential. In general, you should take the lowest dose that works, for the shortest possible time. This will help you go about your daily activities more easily and have a better, more active, quality of life."
            },
            {
                "title": "Side effects of Valium Tablet",
                "description": "Most side effects do not require any medical attention and disappear as your body adjusts to the medicine. Consult your doctor if they persist or if you're worried about them\nCommon side effects of Valium\n• Drowsiness\n• Fatigue\n• Muscle weakness\n• Impaired coordination"
            },
            {
                "title": "How to use Valium Table",
                "description": "Take this medicine in the dose and duration as advised by your doctor. Swallow it as a whole. Do not chew, crush or break it. Valium 10mg Tablet may be taken with or without food."
            },
            {
                "title": "How Valium Tablet works",
                "description": "Valium 10mg Tablet is a benzodiazepine. It works by increasing the action of a chemical messenger (GABA) which suppresses the abnormal and excessive activity of the nerve cells in the brain."
            }
        ]
    },
    {
        "id": "tramadol-citra-100-mg",
        "name": "Tramadol Citra 100 mg",
        "metaTitle": "Buy Tramadol Citra 100 mg Online | Price $499 | Trusted Pharmacy",
        "metaDescription": "Buy Tramadol Citra 100 mg online at best price $499. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Tramadol Citra 100 mg"
        ],
        "image": tramadolCitraImg,
        "images": [
            tramadolCitraImg,
            tramadolCitraImg,
            tramadolCitraImg,
            tramadolCitraImg
        ],
        "boughtCount": "91 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "NOT RETURNABLE",
        "expiry": "2 Year",
        "composition": "Tramadol HCl",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$499",
        "packaging": "180 Units",
        "unitPrice": "$2.77/unit",
        "usage": "Pain Management",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "TRAMADOL HYDROCHLORIDE",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Tramadol is an opioid agonist that may be used to treat moderate to moderately severe chronic pain in adults.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Tramadol is an opioid agonist that may be used to treat moderate to moderately severe chronic pain in adults.\n\nExtended-release forms of tramadol may be used in adults who require around-the-clock treatment of their pain for an extended period. Brands of tramadol include ConZip, Qdolo, Ultram, and Ultram ER. Generic tramadol is available.\n\nTaking tramadol in combination with other pain-relieving medicines such as acetaminophen, ibuprofen, or aspirin can provide better pain relief than just taking one pain-relieving medicine by itself (see Can you take tramadol in combination with acetaminophen, ibuprofen, or aspirin?).\n\nTramadol first gained FDA approval on March 3, 1995. Originally it was classified as a non-controlled substance but it was reclassified in 2014 to Schedule IV of the Controlled Substances Act with restrictions on its prescribing. Tramadol is an opioid (see Is tramadol an opioid?).\n\nA person's genetics can affect the pain-relieving properties of tramadol (see How do genetics affect tramadol?)."
            },
            {
                "title": "Tramadol uses",
                "description": "Tramadol is used for the management of moderate to moderately severe pain in adults, including postoperative pain (pain after surgery).\n\nTramadol treats pain severe enough to require opioids and may be used as an alternative to other forms of pain relief that did not relieve pain well enough or were not tolerated.\n\nTramadol extended-release capsules may be used by adults who require around-the-clock treatment of their moderate to moderately severe chronic pain for an extended time.\n\nTramadol should not be used in children younger than 12 years old, or anyone younger than 18 years old with other risk factors (such as breathing problems), or who have recently had surgery to remove their tonsils or adenoids.\n\nExtended-release tramadol (Ultram ER) should not be given to anyone younger than 18 years."
            },
            {
                "title": "Is Tramadol an opioid?",
                "description": "Yes, tramadol is an opioid. The opioid activity of tramadol is due to its binding to mu-opioid receptors."
            },
            {
                "title": "Tramadol side effects",
                "description": "Common side effects\nThe most common side effects of tramadol are:\n• nausea\n• constipation\n• dry mouth\n• drowsiness\n• dizziness\n• vomiting.\n• A person's genetics can affect their response to tramadol, including their risk of side effects (see How do genetics affect tramadol?)."
            },
            {
                "title": "Serious tramadol side effects and warnings",
                "description": "Tramadol may cause serious side effects, some of which may be fatal.\n\nTramadol can slow or stop your breathing and may be habit-forming. MISUSE OF THIS MEDICINE CAN CAUSE ADDICTION, OVERDOSE, OR DEATH, especially in a child or other person using the medicine without a prescription. Keep this medicine where others cannot get to it. Do not take more tramadol than prescribed as an overdosage can be fatal. Selling or giving away this medicine is against the law.\n\nA person caring for you should give naloxone and seek emergency medical attention if you have slow breathing with long pauses, blue-colored lips, or if you find it hard to wake up. Serious breathing problems may be more likely in older adults and people who are debilitated or have wasting syndrome or chronic breathing disorders.\nCall your doctor at once if you have:\nnoisy breathing, sighing, shallow breathing, breathing that stops during sleep;\na slow heart rate or weak pulse;\na light-headed feeling, like you might pass out;\nseizure (convulsions); or\nlow cortisol levels - nausea, vomiting, loss of appetite, dizziness, worsening tiredness or weakness.\nSeek medical attention right away if you have symptoms of serotonin syndrome, such as agitation, hallucinations, fever, sweating, shivering, fast heart rate, muscle stiffness, twitching, loss of coordination, nausea, vomiting, or diarrhea.\n\nSeizures have been reported in patients taking tramadol. Your risk of seizures is higher if you are taking higher doses than recommended. Seizure risk is also higher in those with a seizure disorder or those taking certain antidepressants or opioid medications.\n\nYou should not take tramadol if you have severe breathing problems, a head injury or increased pressure inside your skull, a blockage in your stomach or intestines, or if you have recently used alcohol, sedatives, tranquilizers, narcotic medication, or an MAO inhibitor (isocarboxazid, linezolid, methylene blue injection, phenelzine, rasagiline, selegiline, tranylcypromine, and others). Tramadol should not be used if you are suicidal or prone to addiction.\n\nFatal side effects can occur if you use this medicine with alcohol, or with other drugs that cause drowsiness or slow your breathing. People taking extended-release forms of tramadol should not consume alcohol-containing beverages at the same time. Alcohol is also not advised with immediate-release forms of tramadol.\n\nSerious allergic reactions, including anaphylaxis, have occurred with tramadol, some of which caused death. People who are allergic to other opioids are at higher risk. Other hypersensitivity reactions include itching, hives, narrowing of the airways, swelling of the face and airways (angioedema), and serious skin reactions such as Stevens-Johnson Syndrome. Get emergency medical help if you have signs of an allergic reaction to tramadol (symptoms include hives, difficulty breathing, swelling in your face or throat) or a severe skin reaction (fever, sore throat, burning in your eyes, skin pain, a red or purple skin rash that spreads and causes blistering and peeling).\n\nTramadol hydrochloride extended-release capsules/tablets may complicate a healthcare provider's clinical assessment of an acute abdominal condition. They should be used by people with liver or kidney disease.\n\nTaking tramadol during pregnancy may cause life-threatening withdrawal symptoms in the newborn.\n\nTramadol should not be given to a child younger than 12 years old, or anyone younger than 18 years old who recently had surgery to remove the tonsils or adenoids. Extended-release tramadol (Ultram ER) should not be given to anyone younger than 18 years.\n\nTramadol may cause drowsiness or affect your ability to drive or perform hazardous tasks. Do not perform these tasks if tramadol causes confusion, mental impairment, or drowsiness.\n\nTramadol can cause withdrawal symptoms if stopped suddenly after being taken for a long period of time. Symptoms may include restlessness, drug craving, anxiety, and sweating (see How long does tramadol withdrawal last?)."
            }
        ]
    },
    {
        "id": "tramadol-trakem-100-mg",
        "name": "Tramadol Trakem 100 mg",
        "metaTitle": "Buy Tramadol Trakem 100 mg Online | Price $499 | Trusted Pharmacy",
        "metaDescription": "Buy Tramadol Trakem 100 mg online at best price $499. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Tramadol Trakem 100 mg"
        ],
        "image": tramadolTrakemImg,
        "images": [
            tramadolTrakemImg,
            tramadolTrakemImg,
            tramadolTrakemImg,
            tramadolTrakemImg
        ],
        "boughtCount": "105 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "NOT RETURNABLE",
        "expiry": "2 Year",
        "composition": "Tramadol HCl",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$499",
        "packaging": "180 Units",
        "unitPrice": "$2.77/unit",
        "usage": "Pain Management",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "TRAMADOL HYDROCHLORIDE",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Tramadol is an opioid agonist that may be used to treat moderate to moderately severe chronic pain in adults.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Tramadol is an opioid agonist that may be used to treat moderate to moderately severe chronic pain in adults.\n\nExtended-release forms of tramadol may be used in adults who require around-the-clock treatment of their pain for an extended period. Brands of tramadol include ConZip, Qdolo, Ultram, and Ultram ER. Generic tramadol is available.\n\nTaking tramadol in combination with other pain-relieving medicines such as acetaminophen, ibuprofen, or aspirin can provide better pain relief than just taking one pain-relieving medicine by itself (see Can you take tramadol in combination with acetaminophen, ibuprofen, or aspirin?).\n\nTramadol first gained FDA approval on March 3, 1995. Originally it was classified as a non-controlled substance but it was reclassified in 2014 to Schedule IV of the Controlled Substances Act with restrictions on its prescribing. Tramadol is an opioid (see Is tramadol an opioid?).\n\nA person's genetics can affect the pain-relieving properties of tramadol (see How do genetics affect tramadol?)."
            },
            {
                "title": "Tramadol uses",
                "description": "Tramadol is used for the management of moderate to moderately severe pain in adults, including postoperative pain (pain after surgery).\n\nTramadol treats pain severe enough to require opioids and may be used as an alternative to other forms of pain relief that did not relieve pain well enough or were not tolerated.\n\nTramadol extended-release capsules may be used by adults who require around-the-clock treatment of their moderate to moderately severe chronic pain for an extended time.\n\nTramadol should not be used in children younger than 12 years old, or anyone younger than 18 years old with other risk factors (such as breathing problems), or who have recently had surgery to remove their tonsils or adenoids.\n\nExtended-release tramadol (Ultram ER) should not be given to anyone younger than 18 years."
            },
            {
                "title": "Is Tramadol an opioid?",
                "description": "Yes, tramadol is an opioid. The opioid activity of tramadol is due to its binding to mu-opioid receptors."
            },
            {
                "title": "Tramadol side effects",
                "description": "Common side effects\nThe most common side effects of tramadol are:\n• nausea\n• constipation\n• dry mouth\n• drowsiness\n• dizziness\n• vomiting.\n• A person's genetics can affect their response to tramadol, including their risk of side effects (see How do genetics affect tramadol?)."
            },
            {
                "title": "Serious tramadol side effects and warnings",
                "description": "Tramadol may cause serious side effects, some of which may be fatal.\n\nTramadol can slow or stop your breathing and may be habit-forming. MISUSE OF THIS MEDICINE CAN CAUSE ADDICTION, OVERDOSE, OR DEATH, especially in a child or other person using the medicine without a prescription. Keep this medicine where others cannot get to it. Do not take more tramadol than prescribed as an overdosage can be fatal. Selling or giving away this medicine is against the law.\n\nA person caring for you should give naloxone and seek emergency medical attention if you have slow breathing with long pauses, blue-colored lips, or if you find it hard to wake up. Serious breathing problems may be more likely in older adults and people who are debilitated or have wasting syndrome or chronic breathing disorders.\nCall your doctor at once if you have:\nnoisy breathing, sighing, shallow breathing, breathing that stops during sleep;\na slow heart rate or weak pulse;\na light-headed feeling, like you might pass out;\nseizure (convulsions); or\nlow cortisol levels - nausea, vomiting, loss of appetite, dizziness, worsening tiredness or weakness.\nSeek medical attention right away if you have symptoms of serotonin syndrome, such as agitation, hallucinations, fever, sweating, shivering, fast heart rate, muscle stiffness, twitching, loss of coordination, nausea, vomiting, or diarrhea.\n\nSeizures have been reported in patients taking tramadol. Your risk of seizures is higher if you are taking higher doses than recommended. Seizure risk is also higher in those with a seizure disorder or those taking certain antidepressants or opioid medications.\n\nYou should not take tramadol if you have severe breathing problems, a head injury or increased pressure inside your skull, a blockage in your stomach or intestines, or if you have recently used alcohol, sedatives, tranquilizers, narcotic medication, or an MAO inhibitor (isocarboxazid, linezolid, methylene blue injection, phenelzine, rasagiline, selegiline, tranylcypromine, and others). Tramadol should not be used if you are suicidal or prone to addiction.\n\nFatal side effects can occur if you use this medicine with alcohol, or with other drugs that cause drowsiness or slow your breathing. People taking extended-release forms of tramadol should not consume alcohol-containing beverages at the same time. Alcohol is also not advised with immediate-release forms of tramadol.\n\nSerious allergic reactions, including anaphylaxis, have occurred with tramadol, some of which caused death. People who are allergic to other opioids are at higher risk. Other hypersensitivity reactions include itching, hives, narrowing of the airways, swelling of the face and airways (angioedema), and serious skin reactions such as Stevens-Johnson Syndrome. Get emergency medical help if you have signs of an allergic reaction to tramadol (symptoms include hives, difficulty breathing, swelling in your face or throat) or a severe skin reaction (fever, sore throat, burning in your eyes, skin pain, a red or purple skin rash that spreads and causes blistering and peeling).\n\nTramadol hydrochloride extended-release capsules/tablets may complicate a healthcare provider's clinical assessment of an acute abdominal condition. They should be used by people with liver or kidney disease.\n\nTaking tramadol during pregnancy may cause life-threatening withdrawal symptoms in the newborn.\n\nTramadol should not be given to a child younger than 12 years old, or anyone younger than 18 years old who recently had surgery to remove the tonsils or adenoids. Extended-release tramadol (Ultram ER) should not be given to anyone younger than 18 years.\n\nTramadol may cause drowsiness or affect your ability to drive or perform hazardous tasks. Do not perform these tasks if tramadol causes confusion, mental impairment, or drowsiness.\n\nTramadol can cause withdrawal symptoms if stopped suddenly after being taken for a long period of time. Symptoms may include restlessness, drug craving, anxiety, and sweating (see How long does tramadol withdrawal last?)."
            }
        ]
    },
    {
        "id": "tramadol-royal-225-mg",
        "name": "Tramadol Royal 225 mg",
        "metaTitle": "Buy Tramadol Royal 225 mg Online | Price $599 | Trusted Pharmacy",
        "metaDescription": "Buy Tramadol Royal 225 mg online at best price $599. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Tramadol Royal 225 mg"
        ],
        "image": tramadolRoyalImg,
        "images": [
            tramadolRoyalImg,
            tramadolRoyalImg,
            tramadolRoyalImg,
            tramadolRoyalImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "REPLACEABLE",
        "expiry": "2 Year",
        "composition": "TRAMADOL HYDROCHLORIDE",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$599",
        "packaging": "180 Tablet",
        "unitPrice": "$3.32/unit",
        "usage": "Pain Management",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "TRAMADOL HYDROCHLORIDE",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Royal is an aggravation medication like a is named a manufactured. It acts in the focal sensory system to ease torment.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Royal is an aggravation medication like a is named a manufactured. It acts in the focal sensory system to ease torment. Royal is utilized to get moderate extreme agony in grown-ups. Royal medication, royal is viewed as helpful as a pain killer with a low potential for misuse. Regardless of these worries, royal is one of numerous normal medicines suggested for osteoarthritis and other agonizing circumstances. The organization of royal with paracetamol was more successful than royal alone for early intense postoperative torment treatment following lumbar discectomy. Consequently, while adding paracetamol in early torment the board is suggested, proceeding paracetamol for the late postoperative period isn't encouraged."
            },
            {
                "title": "Disclaimer",
                "description": "The contents on this is provided solely for informational purposes and is not intended to be a substitute for professional medical advice, diagnosis, or treatment. If you have any questions about a medical problem, please consult a physician or other authorized health expert. Do not dismiss or postpone getting competent medical advice because of something you read on this ."
            }
        ]
    },
    {
        "id": "ritalin-30-mg",
        "name": "Ritalin 30 mg",
        "metaTitle": "Buy Ritalin 30 mg Online | Price $899 | Trusted Pharmacy",
        "metaDescription": "Buy Ritalin 30 mg online at best price $899. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Ritalin 30 mg"
        ],
        "image": ritalinImg,
        "images": [
            ritalinImg,
            ritalinImg,
            ritalinImg,
            ritalinImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "REPLACEABLE",
        "expiry": "2 Year",
        "composition": "METHYLPHENIDATE HYDROCHLORIDE",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$899",
        "packaging": "180 Tablet",
        "unitPrice": "$4.99/unit",
        "usage": "ADHD, narcolepsy",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "METHYLPHENIDATE HYDROCHLORIDE",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Ritalin is a central nervous system stimulant. Methylphenidate affects chemicals in the brain and nerves that contribute to hyperactivity and impulse control.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Ritalin is a central nervous system stimulant. Methylphenidate affects chemicals in the brain and nerves that contribute to hyperactivity and impulse control.\n\nRitalin is used to treat attention deficit disorder (ADD), attention deficit hyperactivity disorder (ADHD), and narcolepsy.\n\nRitalin should be used as a part of a total treatment program for ADHD that may include counseling or other therapies."
            },
            {
                "title": "Warnings",
                "description": "Ritalin may be habit-forming. Tell your doctor if you have a history of drug or alcohol addiction. Keep the medication where others cannot get to it.\n\nMisuse of Ritalin can cause addiction, overdose, or death. Tell your doctor if you have had problems with drug or alcohol abuse.\n\nStimulants have caused stroke, heart attack, and sudden death in people with high blood pressure, heart disease, or a heart defect.\n\nDo not use Ritalin if you have used an MAO inhibitor in the past 14 days, such as isocarboxazid, linezolid, methylene blue injection, phenelzine, rasagiline, selegiline, or tranylcypromine.\n\nRitalin may cause new or worsening psychosis (unusual thoughts or behavior), especially if you have a history of depression, mental illness, or bipolar disorder.\n\nYou may have blood circulation problems that can cause numbness, pain, or discoloration in your fingers or toes.\n\nCall your doctor right away if you have: signs of heart problems - chest pain, feeling light-headed or short of breath; signs of psychosis - paranoia, aggression, new behavior problems, seeing or hearing things that are not real; signs of circulation problems - unexplained wounds on your fingers or toes."
            },
            {
                "title": "Before taking this medicine",
                "description": "You should not use Ritalin if you are allergic to methylphenidate, or if you have:\n\n• glaucoma;\n• severe high blood pressure or a heart problem;\n• overactive thyroid;\n• a personal or family history of tics (muscle twitches) or Tourette's syndrome; or\n• severe anxiety, tension, or agitation (stimulant medicine can make these symptoms worse).\n\nStimulants have caused stroke, heart attack, and sudden death in certain people. Tell your doctor if you have:\n• heart problems or a congenital heart defect;\n• high blood pressure; or\n• a family history of heart disease or sudden death."
            },
            {
                "title": "Dosing information",
                "description": "Usual Adult Dose for Attention Deficit Disorder:\n• IMMEDIATE-RELEASE (IR): Average dose: 20 to 30 mg orally in 2 or 3 divided doses, preferably 30 to 45 minutes before meals; Maximum dose: 60 mg/day.\n• EXTENDED-RELEASE (ER): 8-hour ER oral tablet (e.g., Ritalin SR) - May be used when titrated 8-hour IR dose corresponds to available 8-hour ER tablet; available as 20 mg tablets; Maximum dose: 60 mg/day\n\nUsual Adult Dose for Narcolepsy:\n• IMMEDIATE-RELEASE (IR): Average dose: 20 to 30 mg orally in 2 or 3 divided doses, preferably 30 to 45 minutes before meals; Maximum dose: 60 mg/day.\n• EXTENDED-RELEASE (ER): 8-hour ER oral tablet (e.g., Ritalin SR) - May be used when titrated 8-hour IR dose corresponds to available 8-hour ER tablet; available as 20 mg tablets; Maximum dose: 60 mg/day."
            },
            {
                "title": "Ritalin side effects",
                "description": "Get emergency medical help if you have signs of an allergic reaction to Ritalin: hives; difficulty breathing; swelling of your face, lips, tongue, or throat.\n\nCall your doctor at once if you have:\n• signs of heart problems - chest pain, trouble breathing, feeling like you might pass out;\n• signs of psychosis - hallucinations (seeing or hearing things that are not real), new behavior problems, aggression, hostility, paranoia;\n• signs of circulation problems - numbness, pain, cold feeling, unexplained wounds, or skin color changes (pale, red, or blue appearance) in your fingers or toes; or\n• penis erection that is painful or lasts 4 hours or longer.\n\nSeek medical attention right away if you have symptoms of serotonin syndrome, such as: agitation, hallucinations, fever, sweating, shivering, fast heart rate, muscle stiffness, twitching, loss of coordination, nausea, vomiting, or diarrhea.\nMethylphenidate can affect growth in children. Your child's height and weight may need to be checked often. Tell your doctor if your child is not growing at a normal rate.\n\nCommon Ritalin side effects include:\n• sweating, increased blood pressure;\n• mood changes, anxiety, feeling nervous or irritable, trouble sleeping;\n• fast heart rate, pounding heartbeats or fluttering in your chest;\n• loss of appetite, weight loss;\n• dry mouth, nausea, vomiting, stomach pain, indigestion; or\n• headache, dizziness."
            }
        ]
    },
    {
        "id": "tapentadol-100-mg",
        "name": "Tapentadol (Aspadol-100mg)",
        "metaTitle": "Buy Tapentadol (Aspadol-100mg) Online | Price $599 | Trusted Pharmacy",
        "metaDescription": "Buy Tapentadol (Aspadol-100mg) online at best price $599. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "Tapentadol (Aspadol-100mg)"
        ],
        "image": tapentadolImg,
        "images": [
            tapentadolImg,
            tapentadolImg,
            tapentadolImg,
            tapentadolImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "REPLACEABLE",
        "expiry": "2 Year",
        "composition": "TAPENTADOL",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$599",
        "packaging": "180 Tablet",
        "unitPrice": "$3.32/unit",
        "usage": "Pain relief",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "TAPENTADOL",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Aspadol Tablet is a medicine used to treat moderate to severe acute pain in adults.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Aspadol Tablet is a medicine used to treat moderate to severe acute pain in adults. It is used to treat many conditions such as headache, fever, period pain, toothache, and colds. It effectively alleviates pain when other treatments fail to relieve your pain."
            },
            {
                "title": "Uses of Aspadol Tablet",
                "description": "• Moderate to severe pain"
            },
            {
                "title": "Benefits of Aspadol Tablet",
                "description": "In Moderate to severe pain\nAspadol Tablet helps to relieve moderate to severe pain like toothache, headache, or in conditions that affect joints and muscles. It works by blocking chemical messengers in the brain that tell us we have pain. It can help relieve pain in conditions like rheumatoid arthritis and osteoarthritis. Take it as it is prescribed to get the most benefit. Do not take more or for longer than needed as that can be dangerous. In general, you should take the lowest dose that works, for the shortest possible time. This will help you to go about your daily activities more easily and have a better, more active, quality of life."
            },
            {
                "title": "Side effects of Aspadol Tablet",
                "description": "Most side effects do not require any medical attention and disappear as your body adjusts to the medicine. Consult your doctor if they persist or if you’re worried about them\nCommon side effects of Aspadol\n• Nausea\n• Vomiting\n• Dizziness\n• Sleepiness"
            },
            {
                "title": "How to use Aspadol Tablet",
                "description": "Take this medicine in the dose and duration as advised by your doctor. Swallow it as a whole. Do not chew, crush or break it. Aspadol Tablet may be taken with or without food, but it is better to take it at a fixed time."
            },
            {
                "title": "How Aspadol Tablet works",
                "description": "Aspadol Tablet works by changing how the brain and nerves respond to pain. It reduces pain signals by acting on opioid receptors and also enhances the levels of a natural chemical called norepinephrine, which helps block pain messages in the nervous system. Together, these actions help provide effective relief from moderate to severe pain."
            }
        ]
    },
    {
        "id": "firocit",
        "name": "FIROCIT",
        "metaTitle": "Buy FIROCIT Online | Price $499 | Trusted Pharmacy",
        "metaDescription": "Buy FIROCIT online at best price $499. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "FIROCIT"
        ],
        "image": firocitImg,
        "images": [
            firocitImg,
            firocitImg,
            firocitImg,
            firocitImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "REPLACEABLE",
        "expiry": "2 Year",
        "composition": "FUMARATE",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$499",
        "packaging": "180 Tablet",
        "unitPrice": "$2.77/unit",
        "usage": "Anemia prevention",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "FUMARATE",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Ferocit TR is indicated for the prevention and treatment of iron and Folic Acid deficiencies specially during pregnancy and lactation.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Ferocit TR is indicated for the prevention and treatment of iron and Folic Acid deficiencies specially during pregnancy and lactation."
            },
            {
                "title": "Dosage and Administration",
                "description": "One capsule daily."
            },
            {
                "title": "Composition",
                "description": "Dried Ferrous Sulfate BP 150 mg (equivalent to 45 - 47 mg iron) and Folic Acid BP 0.5 mg in timed release formulation."
            }
        ]
    },
    {
        "id": "gabapentin",
        "name": "GABAPENTENE-",
        "metaTitle": "Buy GABAPENTENE- Online | Price $599 | Trusted Pharmacy",
        "metaDescription": "Buy GABAPENTENE- online at best price $599. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "GABAPENTENE-"
        ],
        "image": gabapenteneImg,
        "images": [
            gabapenteneImg,
            gabapenteneImg,
            gabapenteneImg,
            gabapenteneImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "REPLACEABLE",
        "expiry": "2 Year",
        "composition": "GABASIGN-800",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$599",
        "packaging": "180 Tablet",
        "unitPrice": "$3.32/unit",
        "usage": "Neuropathic pain, seizures",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "GABASIGN-800",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Gabapentin is a prescription medication known as a gamma aminobutyric acid (GABA) analogue.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Gabapentin is a prescription medication known as a gamma aminobutyric acid (GABA) analogue. GABA reduces the excitability of nerve cells (neurons) in the brain, which play a role in seizures and the transmission of pain signals. Gabapentin mirrors the effects of GABA calming excited neurons.\n\nGabapentin is in a class of medications called anticonvulsants."
            },
            {
                "title": "Uses of gabapentin Tablet",
                "description": "Gabapentin is used to:\n• Prevent and control partial seizures. Gabapentin can be used in adults and children age 3 and older who have partial seizures.\n• Relieve nerve pain following shingles in adults. Shingles is a painful rash that develops many years after you've had chickenpox. The virus that causes chickenpox stays dormant in a portion of your spinal nerve root called the dorsal root ganglion. For whatever reason, this otherwise dormant virus gets reactivated — usually by stress — causing a shingles rash. Nerve pain following a case of shingles is called postherpetic neuralgia (PHN).\n• Treat moderate-to-severe primary restless legs syndrome.\nThe branded gabapentin products Neurontin and Gralise are approved for partial seizures and PHN. The branded gabapentin enacarbil product Horizant is approved for restless legs syndrome and PHN."
            },
            {
                "title": "Side effects of gabapentin Tablet",
                "description": "Common side effects of gabapentin include:\n• Feeling tired.\n• Dizziness.\n• Headache.\n• Nausea and vomiting.\n• Fever.\n• Difficulty speaking.\n• Recurring infections.\n• Memory loss.\n• Weight gain.\n• Movement problems: coordination problems, being unsteady, tremors, jerky movements.\n• Eye problems: unusual eye movements, double vision.\nTalk to your healthcare provider if any side effects do not go away."
            },
            {
                "title": "How to use gabapentin Tablet",
                "description": "• Take Gralise tablets with your evening meal. Swallow tablets whole with a full glass of water. Don't chew, break or crush.\n• Take Horizant tablets with food. Swallow tablets whole with a full glass of water. Don't chew, break or crush.\n• Take other forms of gabapentin with or without food.\n• Neurontin and generic forms of Neurontin tablets may be broken into two pieces. You can take the second half for your next dose. Don't use the half-tablet beyond 28 days after the whole tablet was cut or broken.\n• Carefully measure the liquid formulation of gabapentin using the measuring device that comes with the drug. If you did not receive a measuring device, please ask your pharmacist for a medication-measuring device.\n• If you take an aluminum or magnesium-containing antacid, such as Maalox®, Mylanta®, Gelusil®, Gaviscon®, or Di-Gel®, wait at least two hours before taking your next dose of gabapentin.\n• Take gabapentin exactly as prescribed by your healthcare provider."
            }
        ]
    },
    {
        "id": "ativan",
        "name": "ATIVAN (LOREZAPAM)",
        "metaTitle": "Buy ATIVAN (LOREZAPAM) Online | Price $699 | Trusted Pharmacy",
        "metaDescription": "Buy ATIVAN (LOREZAPAM) online at best price $699. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "ATIVAN (LOREZAPAM)"
        ],
        "image": ativanImg,
        "images": [
            ativanImg,
            ativanImg,
            ativanImg,
            ativanImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "REPLACEABLE",
        "expiry": "2 Year",
        "composition": "LOREZAPAM",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$699",
        "packaging": "180 Tablet",
        "unitPrice": "$3.88/unit",
        "usage": "Anxiety",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "LOREZAPAM",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Ativan 2mg Tablet is a prescription medicine used to treat symptoms of short term anxiety and anxiety disorders.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Ativan 2mg Tablet is a prescription medicine used to treat symptoms of short term anxiety and anxiety disorders. It helps to decrease the abnormal and excessive activity of the nerve cells and calms the brain.\n\nAtivan 2mg Tablet is taken by mouth with or without food. Take it at the same time each day as this helps to maintain a consistent level of medicine in the body. Take this medicine in the dose and duration as advised by your doctor as it has a high potential of habit-forming. If you have missed a dose, take it as soon as you remember it and finish the full course of treatment even if you feel better. It is important that this medication is not stopped suddenly without talking to the doctor as it may increase seizure frequency.\n\nSome common side effects of this medicine include fatigue and balance disorder. It may also cause dizziness and sleepiness, so do not drive or do anything that requires mental focus until you know how this medicine affects you."
            },
            {
                "title": "Uses of Ativan Tablet",
                "description": "• Treatment of Anxiety disorder\n• Treatment of Short term anxiety"
            },
            {
                "title": "Benefits of Ativan Tablet",
                "description": "In Treatment of Anxiety disorder\nAtivan 2mg Tablet helps relieve symptoms of many anxiety disorders including obsessive-compulsive disorder and generalised anxiety disorder by increasing the level of a chemical called GABA in your brain. This prevents abnormal symptoms of anxiety such as restlessness, tiredness, difficulty concentrating, feeling irritable and sleep problems. It helps you feel calm with a better ability to deal with problems. Exercise and a healthy diet can also improve your mood. Keep taking the medicine until your doctor advises you to stop."
            },
            {
                "title": "Side effects of Ativan Tablet",
                "description": "Most side effects do not require any medical attention and disappear as your body adjusts to the medicine. Consult your doctor if they persist or if you're worried about them\nCommon side effects of Ativan\n• Drowsiness\n• Dizziness\n• Injection site pain\n• Sedation\n• Fatigue\n• Muscle weakness\n• Unsteadiness"
            },
            {
                "title": "How to use Ativan Tablet",
                "description": "Take this medicine in the dose and duration as advised by your doctor. Swallow it as a whole. Do not chew, crush or break it. Ativan 2mg Tablet may be taken with or without food."
            },
            {
                "title": "How Ativan Tablet works",
                "description": "Ativan 2mg Tablet is a benzodiazepine that works by enhancing the effect of a natural chemical in the brain called gamma-aminobutyric acid (GABA). GABA has an inhibitory effect on brain activity, helping to calm excessive nerve signals that contribute to anxiety. Binding to specific sites on the GABA-A receptor, Ativan 2mg Tablet increases the receptor's response to GABA, leading to a sedative, anxiolytic (anti-anxiety), and calming effect. This mechanism helps relieve symptoms of anxiety disorders by reducing feelings of nervousness, agitation, and restlessness."
            }
        ]
    },
    {
        "id": "soma-350-mg",
        "name": "SOMA 350MG",
        "metaTitle": "Buy SOMA 350MG Online | Price $699 | Trusted Pharmacy",
        "metaDescription": "Buy SOMA 350MG online at best price $699. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "SOMA 350MG"
        ],
        "image": somaImg,
        "images": [
            somaImg,
            somaImg,
            somaImg,
            somaImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "REPLACEABLE",
        "expiry": "2 Year",
        "composition": "CARISOPRODOL",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$699",
        "packaging": "180 Tablet",
        "unitPrice": "$3.88/unit",
        "usage": "Muscle relaxation",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "CARISOPRODOL",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Carisoprodol is used for muscle relaxation. It relieves painful spasms of the skeletal muscles.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Carisoprodol is used for muscle relaxation. It relieves painful spasms of the skeletal muscles."
            },
            {
                "title": "How Carisoprodol works",
                "description": "Carisoprodol is a muscle relaxant. It works on the centres in the brain and spinal cord to relieve muscle stiffness or spasm without reduction in strength. This improves pain and movement of muscles."
            },
            {
                "title": "Common side effects of Carisoprodol",
                "description": "Most side effects do not require any medical attention and disappear as your body adjusts to the medicine. Consult your doctor if they persist or if you're worried about them\n\nCommon\nSleepiness, Headache, Dizziness"
            }
        ]
    },
    {
        "id": "oxycodone-30-mg",
        "name": "OXYCODONE -30MG",
        "metaTitle": "Buy OXYCODONE -30MG Online | Price $799 | Trusted Pharmacy",
        "metaDescription": "Buy OXYCODONE -30MG online at best price $799. Trusted prescription medicine for safe and effective treatment. Discreet shipping available.",
        "breadcrumbs": [
            "Home",
            "Products",
            "OXYCODONE -30MG"
        ],
        "image": oxycodoneImg,
        "images": [
            oxycodoneImg,
            oxycodoneImg,
            oxycodoneImg,
            oxycodoneImg
        ],
        "boughtCount": "63 people bought in last 7 days",
        "isPrescription": true,
        "consumeType": "ORAL",
        "returnPolicy": "REPLACEABLE",
        "expiry": "2 Year",
        "composition": "ROXICODONE",
        "manufacturer": "PHARMACEUTICALS LTD",
        "mrp": "$799",
        "packaging": "180 Tablet",
        "unitPrice": "$4.43/unit",
        "usage": "Opioid analgesic management of pain",
        "specs": {
            "Consume Type": "ORAL",
            "Return Policy": "REPLACEABLE",
            "Expires on or after": "March 2027",
            "Composition": "ROXICODONE",
            "Quantity": "VARIES (COMMONLY AVAILABLE AS 10 TABLETS PER STRIP)",
            "Dosage": "AS PRESCRIBED BY PHYSICIAN, 5 TO 10 MG BEFORE BEDTIME"
        },
        "description": "Oxycodone is an opioid used in the management of moderate to severe pain.",
        "uses": [
            {
                "title": "Product introduction",
                "description": "Oxycodone is an opioid used in the management of moderate to severe pain."
            },
            {
                "title": "INDICATIONS AND USAGE",
                "description": "ROXICODONE (oxycodone HCl) tablets are indicated for the management of pain severe enough to require an opioid analgesic and for which alternative treatments are inadequate.\n\nLimitations of Use\nBecause of the risks of addiction, abuse, and misuse with opioids, even at recommended doses, reserve ROXICODONE for use in patients for whom alternative treatment options (e.g., non-opioid analgesics or opioid combination products):\n• Have not been tolerated, or are not expected to be tolerated,\n• Have not provided adequate analgesia, or are not expected to provide adequate analgesia."
            },
            {
                "title": "IMPORTANT SAFETY INFORMATION",
                "description": "WARNING: ADDICTION, ABUSE, AND MISUSE; RISK EVALUATION AND MITIGATION STRATEGY (REMS); LIFE-THREATENING RESPIRATORY DEPRESSION; ACCIDENTAL INGESTION; NEONATAL OPIOID WITHDRAWAL' SYNDROME; CYTOCHROME P450 3A4 INTERACTION; and RISKS FROM CONCOMITANT USE WITH BENZODIAZEPINES OR OTHER CNS DEPRESSANTS\n\nAddiction, Abuse, and Misuse\nROXICODONE expose patients and other users to the risks of opioid addiction, abuse, and misuse, which can lead to overdose and death. Assess each patient’s risk prior to prescribing ROXICODONE, and monitor all patients regularly for the development of these behaviors and conditions.\n\nOpioid Analgesic Risk Evaluation and Mitigation Strategy (REMS)\nTo ensure that the benefits of opioid analgesics outweigh the risks of addiction, abuse, and misuse, the Food and Drug Administration (FDA) has required a REMS for these products. Under the requirements of the REMS, drug companies with approved opioid analgesic products must make REMS-compliant education programs available to healthcare providers. Healthcare providers are strongly encouraged to • complete a REMS-compliant education program, • counsel patients and/or their caregivers, with every prescription, on safe use, serious risks, storage, and disposal of these products, • emphasize to patients and their caregivers the importance of reading the Medication Guide every time it is provided by their pharmacist, and • consider other tools to improve patient, household, and community safety.\n\nLife-Threatening Respiratory Depression\nSerious, life-threatening, or fatal respiratory depression may occur with use of ROXICODONE. Monitor for respiratory depression, especially during initiation of ROXICODONE or following a dose increase.\n\nAccidental Ingestion\nAccidental ingestion of ROXICODONE, especially by children, can result in a fatal overdose of ROXICODONE.\n\nNeonatal Opioid Withdrawal Syndrome\nProlonged use of ROXICODONE during pregnancy can result in neonatal opioid withdrawal syndrome, which may be life-threatening if not recognized and treated, and requires management according to protocols developed by neonatology experts. If opioid use is required for a prolonged period in a pregnant woman, advise the patient of the risk of neonatal opioid withdrawal syndrome and ensure that appropriate treatment will be available.\n\nCytochrome P450 3A4 Interaction\nThe concomitant use of ROXICODONE with all cytochrome P450 3A4 inhibitors may result in an increase in oxycodone plasma concentrations, which could increase or prolong adverse reactions and may cause potentially fatal respiratory depression. In addition, discontinuation of a concomitantly used cytochrome P450 3A4 inducer may result in an increase in oxycodone plasma concentration. Monitor patients receiving ROXICODONE and any CYP3A4 inhibitor or inducer.\n\nRisks from Concomitant Use with Benzodiazepines or Other CNS Depressants\nConcomitant use of opioids with benzodiazepines or other central nervous system (CNS) depressants, including alcohol, may result in profound sedation, respiratory depression, coma, and death.\nReserve concomitant prescribing of ROXICODONE and benzodiazepines or other CNS depressants for use in patients for whom alternative treatment options are inadequate.\nLimit dosages and durations to the minimum required.\nFollow patients for signs and symptoms of respiratory depression and sedation."
            }
        ]
    }
];

// Trending Section Data (Sliced from allProducts)
export const trendingProducts = allProducts.slice(0, 8).map(p => ({
    id: p.id,
    name: p.name,
    usedFor: p.usage || (p.uses && p.uses[0] ? p.uses[0].description : ""),
    price: p.mrp,
    image: p.image
}));

export const trendingCategories = ["Medicine", "Vitamins", "Supplements", "Health Care"];

// Recommended Section Data (Sliced & mapped)
export const recommendedProducts = allProducts.slice(8, 14).map(p => ({
    id: p.id,
    category: p.usage || "Health",
    name: p.name,
    specs: p.packaging + " • " + p.expiry,
    price: p.unitPrice.split('/')[0],
    image: p.image
}));

// Latest News Data
export const newsItems = [
    {
        date: "August 11,2025",
        category: "US Medicine",
        title: "In this section, we delve into various aspects of health",
        description: "Explore the world of medical specialties through our blog's spotlight feature. From...",
        tags: ["Examination gloves", "Uncategorized"]
    },
    {
        date: "August 11,2025",
        category: "US Medicine",
        title: "Discover a treasure trove of practical tips for enhancing",
        description: "From nutrition advice to exercise routines, we're here to support your journey toward...",
        tags: ["Examination gloves", "Uncategorized"]
    },
    {
        date: "August 11,2025",
        category: "US Medicine",
        title: "Our patients' journeys are filled with courage, resilienc...",
        description: "In this section, we share inspiring narratives of individuals who have overcome health c...",
        tags: ["Examination gloves", "Uncategorized"]
    },
    {
        date: "August 11,2025",
        category: "US Medicine",
        title: "From organizing health fairs to partnering with local organiz...",
        description: "Find out how you can participate in community events and contribute to the h...",
        tags: ["Examination gloves", "Uncategorized"]
    }
];

// Process Steps Data
export const processSteps = [
    {
        number: 1,
        title: "Send Query",
        description: "Browse medicines, add to query cart, and send us your request.",
        color: "bg-[#FFF4D9] text-[#AD842A]"
    },
    {
        number: 2,
        title: "Order Confirmation",
        description: "Our team will contact you to confirm availability.",
        color: "bg-[#DFF6FF] text-[#2A84AD]"
    },
    {
        number: 3,
        title: "Secure Payment",
        description: "Once confirmed, complete your payment securely online.",
        color: "bg-[#E4F8D7] text-[#5B8D2A]"
    },
    {
        number: 4,
        title: "Delivery",
        description: "our medicines will be packed and delivered to your doorstep.",
        color: "bg-[#EBEBFF] text-[#5B5BBD]"
    }
];

// Banner Grid Data (IDs for mapping)
export const bannerGridItems = [1, 2, 3];

// Default product details fallback (First item)
export const productDetails = allProducts[0];
